...
abstract: |
  ...
  Yaptığımız analizde kazanılan skorlar arasında anlamlı pozitif korelasyon olduğunu gördük. Normal olarak olması gereken gibi kaçırılan toplarla da skor arasında negatif korelasyon olduğunu bulduk. Zaman açısından yıllara göre oyunlardaki toplam skorun medyan değerine baktığımızda da gözeçarpan bir değişiklik olduğu söylenemez. 
---
...
Tablo \ref{tab:ozet}'da göründüğü gibi maksimum skor 398 puan ve minimum ise 8 puandır. Skor 1 ortalamsı 233 puan standart sapması ise 56 puan olarak gözüküyor. Medyan değeri ise 237 ile ortalamaya oldukça yakın. Aynı şekilde skor 2 değişkeninde ortalama 200 ve yine medyan değeri 203 ile ortalama ve medyan değerleri bir-birine çok yakındır. Kenara giden toplara bakarsak, tabii ki, maksimum olabilecek maksimum sayı 10 ve minimumsa 0'dır. Aynı şekilde skorda olduğu gibi burda da ortalamalarla medyan değerleri yakındır. Ortalamada çok puan olmasına rağmen 1-ci takımlarda  kenara giden top ortalaması ve medyanı fazladır. Yani savunucular görevlerini yerine getirmektedirler. Kaleye giden topun da tahtaya isabet etmesi çok düşük ihtimaldir. Total skora bakacak olursak da yine en düşük skorlu maçta toplam 73, en yüksek skorlu maçta ise skorun 693 olduğunu görürüz. Total skor açısından da medyan değeri 441 puandır.
...
Şekil \@ref(fig:plot)'de skor 1 değişkeninin neden daha fazla ortalamaya sahip olduğunu görüyoruz. Ekseriyette fark açılan oyunlarda kazanan takımın skoru skor 1 sütununda yer almaktadır. Sol üst köşenin boş olmasından ve sağ alt köşede daha çok nokta olmasından bunu anlaya biliriz. Skoru 400'e yakın olan maçlar da genellikle skor 1dedir. Regresyon doğrusunun aynı düzeyde devam etmemesi de yüksek olan skorların skor1'de yazılmasından kaynaklanmaktadır.
...
Şekil \@ref(fig:plot1)'deki yoğunluk grafiğine bakarsak en fazla oranla maçların toplamda 380-520 skorla bittiğini göre biliriz. Dağılım sola doğru çarpık olarak gözüküyor. Oyunda sayı 200'den az ve 640'dan fazla skor olanlar genelin azını teşkil etmektedir. Dağılım normal dağılıma benziyor. Tablo \ref{tab:sha}'deki Shapiro testinden de toplam skorun normal olarak dağıldığını görüyoruz.
...
formatted_table <- kable(df, caption = "Shapiro-Wilk Normallik Testi") %>%
  kable_styling()
...
Şekil \@ref(fig:plot2)'da yıllar bazında en yüksek ortalama skorun verinin toplandığı en son yıla denk geldiğini görüyoruz. En düşük skorlu yıl 2003 yılına aittir. Genel olarak ortalama skorlar pek farklılaşmamakla birlikte yıllar bazında da önemli bir çizgi göremiyoruz.

Tabii ki, kenara giden top sayısı arttıkça skor kazanmak da takım açısından zorlaşıyor. Skorla kaçırılan top sayı arasında korelasyona baktığımızda istatistiksel açıdan anlamlı korelasyon katsayısını -0.61 olarak buluyoruz. Yani bu iki değişken arasında ters yönlü bir ilişki vardır.

```{r plot2, echo=FALSE, fig.align='center', fig.cap='Yıllara göre Ortalama Skorlar', fig.height=5, fig.width=8}

data %>%
  group_by(year) %>%
  summarize(mean_total_score = mean(total_skor)) %>%
  mutate(Year = fct_reorder(year, mean_total_score)) %>%
  ggplot(aes(mean_total_score, Year, fill = mean_total_score)) +
  geom_col() 
...
kable(df, caption = "Korelasyon Test Sonuçları")
